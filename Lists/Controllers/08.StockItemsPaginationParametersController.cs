...
namespace devMobile.WebAPIDapper.Lists.Controllers
{
	...
	public class StockItemsPaginationParametersController : ControllerBase
	{
		...
		private readonly ILogger<StockItemsPaginationParametersController> logger;

		public StockItemsPaginationParametersController(IConfiguration configuration, ILogger<StockItemsPaginationParametersController> logger)
		{
		...
		public async Task<ActionResult<IAsyncEnumerable<Model.StockItemListDtoV1>>> Get([FromQuery] Model.StockItemPagingDtoV1 request)
		{
			...
			{
				logger.LogError(ex, "StockItemsPaginationParameters exception retrieving list of StockItems with PageSize:{0} PageNumber:{1}", request.PageSize, request.PageNumber);
