...
namespace devMobile.WebAPIDapper.Swagger.Controllers
{
    ...
    public class InvoiceController : ControllerBase
    {
        ...
        [HttpGet("{invoiceId}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type=typeof(Swagger.Models.InvoiceSummaryGetDtoV1))]
        public async Task<ActionResult<Swagger.Models.InvoiceSummaryGetDtoV1>> Get([Required][Range(1, int.MaxValue, ErrorMessage = "Invoice id must greater than 0")] int invoiceId)
        {
            Swagger.Models.InvoiceSummaryGetDtoV1 response ;
            ...
            using (SqlConnection db = new SqlConnection(this.connectionString))
            {
                ...
                response = await invoiceSummary.ReadSingleOrDefaultWithRetryAsync<Swagger.Models.InvoiceSummaryGetDtoV1>();
                if (response == default)
                {
                ...
                response.InvoiceLines = (await invoiceSummary.ReadWithRetryAsync<Swagger.Models.InvoiceLineSummaryListDtoV1>()).ToArray();
